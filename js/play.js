...
play.init = function (depth, map){
	...
	play.checkAudio = new Audio("wav/1.wav")
...
play.isCheck = function() {
    var kingPos, opponentKingPos;
    
    // 找到双方的将（帅）位置
    for (var key in play.mans) {
        if (play.mans[key] && play.mans[key].isShow) {
            if (key === "j0") kingPos = { x: play.mans[key].x, y: play.mans[key].y };   // 红方将
            if (key === "J0") opponentKingPos = { x: play.mans[key].x, y: play.mans[key].y }; // 黑方帅
        }
    }

    // 检测红方是否将军
    if (play.isKingInCheck(-1, kingPos)) {
		play.checkAudio.play();
        alert("红方被将军！");
        return true;
    }
    
    // 检测黑方是否将军
    if (play.isKingInCheck(1, opponentKingPos)) {
		play.checkAudio.play();
        alert("黑方被将军！");
        return true;
    }
...
// 检测指定的将（帅）是否在当前局面下被将军
play.isKingInCheck = function(my, kingPos) {
    for (var key in play.mans) {
        var man = play.mans[key];
        if (man && man.isShow && man.my !== my) { // 遍历敌方所有棋子
            var moves = man.bl(); // 获取该棋子所有可能的走法
            for (var i = 0; i < moves.length; i++) {
                if (moves[i][0] === kingPos.x && moves[i][1] === kingPos.y) {
                    return true; // 棋子的某一步可以攻击到将或帅
                }
            }
        }
    }
